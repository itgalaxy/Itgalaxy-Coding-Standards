...
class DoubleQuoteUsageSniff implements \PHP_CodeSniffer_Sniff
{
    ...
    public function process(\PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === T_DOUBLE_QUOTED_STRING) {
            ...
            if (is_string($this->allowedVariables)) {
                $this->allowedVariables = explode(',', $this->allowedVariables);
            }

            foreach ($stringTokens as $token) {
                if (is_array($token) === true && $token[0] === T_VARIABLE) {
                    if (in_array($token[1], $this->allowedVariables)) {
                        return;
                    }
        ...
        if (is_string($this->additionalAllowedChars)) {
            $this->additionalAllowedChars = explode(',', $this->additionalAllowedChars);
        }

        $this->allowedChars = array_unique(
            array_merge($this->allowedChars, (array) ($this->additionalAllowedChars))
        );

        foreach ($this->allowedChars as $testChar) {
