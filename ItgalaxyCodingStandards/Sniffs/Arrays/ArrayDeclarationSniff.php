...
class ArrayDeclarationSniff implements \PHP_CodeSniffer_Sniff
{
    ...
    public function processMultiLineArray(\PHP_CodeSniffer_File $phpcsFile, $stackPtr, $arrayStart, $arrayEnd)
    {
        ...
        if ($tokens[$lastContent]['line'] === $tokens[$arrayEnd]['line']) {
            ...
            $error = 'Closing parenthesis or bracket not aligned correctly; expected %s space(s) but found %s';
            $data = [
                $expected,
                $found
            ];
            ...
            if ($fix === true) {
                if ($found === 0) {
                    $phpcsFile
                        ->fixer
                        ->addContent(($arrayEnd - 1), str_repeat(' ', $expected));
                } else {
                    $phpcsFile
                        ->fixer
                        ->replaceToken(($arrayEnd - 1), str_repeat(' ', $expected));
                }
            }
        }
        ...
        for ($nextToken = ($stackPtr + 1); $nextToken < $arrayEnd; $nextToken++) {
            ...
            if (isset($tokens[$nextToken - 1]) && $tokens[$nextToken - 1]['code'] === T_WHITESPACE
                && $tokens[$nextToken]['code'] === T_WHITESPACE
                && isset($tokens[$nextToken + 1]) && $tokens[$nextToken + 1]['code'] === T_WHITESPACE
            ) {
                $error = 'No spaces between values or keys of array';
                $phpcsFile->addError($error, $nextToken, 'NoSpaceBetweenValueOrKeys');
            }
            ...
            if ($tokens[$nextToken]['code'] === T_DOUBLE_ARROW) {
                ...
                if ($indexStart === $indexEnd) {
                    ...
                    $currentEntry['index_content']
                        = $phpcsFile->getTokensAsString($indexStart, ($indexEnd - $indexStart + 1));
                }
        ...
        if (empty($indices) === true) {
            ...
            $nextContent = $phpcsFile->findNext($exclude, $lastToken + 1, $arrayEnd, true);
            $indices[] = [
                'value' => $nextContent
            ];
        ...
        */
        ...
        if (empty($indices) === false) {
            $expectedIndent = (($statementStart - 1) + $this->indent);

            if ($keyUsed === true) {
                $startPtr = $indices[0]['index'];
            } else {
                $startPtr = $indices[0]['value'];
            }

            $foundIndent = $tokens[$startPtr]['column'] - 1;

            if ($foundIndent !== $expectedIndent) {
                $error = 'First key or value of multi-line array not indented correctly; '
                    . 'expected %s spaces but found %s';
                $data = [
                    $expectedIndent,
                    $foundIndent
                ];
                $phpcsFile->addError($error, $startPtr, 'ArrayIndent', $data);
            }
            ...
            if ($tokens[$trailingContent]['code'] === T_COMMA) {
                ...
                $error = 'Comma not required after last value in array declaration';
                $fix = $phpcsFile->addFixableError($error, $trailingContent, 'CommaAfterLast');

                if ($fix === true) {
                    $phpcsFile
                        ->fixer
                        ->addContent($trailingContent, ',');
                }
            } else {
                ...
                if ($lastValueLine !== false && $tokens[$value['value']]['line'] === $lastValueLine) {
                    ...
                    if ($fix === true) {
                        if ($tokens[($value['value'] - 1)]['code'] === T_WHITESPACE) {
                            $phpcsFile
                                ->fixer
                                ->replaceToken(($value['value'] - 1), '');
                        }

                        $phpcsFile
                            ->fixer
                            ->addNewlineBefore($value['value']);
                    }
                    ...
                    if ($found !== $expected) {
                        ...
                        if ($fix === true) {
                            if ($found === 0) {
                                $phpcsFile
                                    ->fixer
                                    ->addContent(($value['value'] - 1), str_repeat(' ', $expected));
                            } else {
                                $phpcsFile
                                    ->fixer
                                    ->replaceToken(($value['value'] - 1), str_repeat(' ', $expected));
                            }
                        }
                    }
                }
        ...
        */
        ...
        foreach ($indices as $index) {
            if (isset($index['index']) === false) {
                // Todo indent bugs :( for seconds and late keys
                if ($lastColumn !== null && $lastColumn !== $tokens[$index['value']]['column']) {
                    $expected = $lastColumn - 1;
                    $found = $tokens[$index['value']]['column'] - 1;
                    $error = 'Array values not correctly spaces; expected %s spaces but found %s';
                    $data = [
                        $expected,
                        $found
                    ];

                    $phpcsFile->addError($error, $index['value'], 'ValueNotCorrectlySpaces', $data);
                }

                $lastColumn = $tokens[$index['value']]['column'];
            ...
            if ($lastColumn !== null && $lastColumn !== $tokens[$index['index']]['column']) {
                $expected = $lastColumn - 1;
                $found = $tokens[$index['index']]['column'] - 1;
                $error = 'Array key not correctly spaces; expected %s spaces but found %s';
                $data = [
                    $expected,
                    $found
                ];

                $phpcsFile->addError($error, $index['value'], 'KeyNotCorrectlySpaces', $data);
            }

            $lastColumn = $tokens[$index['index']]['column'];
            ...
            if ($tokens[$index['index']]['code'] !== T_LNUMBER
                && $tokens[$index['index']]['code'] !== T_CONSTANT_ENCAPSED_STRING
                && $tokens[$index['index']]['code'] !== T_STRING
            ) {
                $error = 'Array key must be string or number';
                $phpcsFile->addError($error, $index['index'], 'KeyTypeNotAllowed');
            }
            ...
            if ($tokens[$index['index']]['column'] !== $indicesStart) {
                ...
                if ($fix === true) {
                    if ($found === 0) {
                        $phpcsFile
                            ->fixer
                            ->addContent(($index['index'] - 1), str_repeat(' ', $expected));
                    } else {
                        $phpcsFile
                            ->fixer
                            ->replaceToken(($index['index'] - 1), str_repeat(' ', $expected));
                    }
                }
            ...
            if ($tokens[$index['arrow']]['column'] !== $arrowStart) {
                ...
                if ($fix === true) {
                    if ($found === 0) {
                        $phpcsFile
                            ->fixer
                            ->addContent(($index['arrow'] - 1), str_repeat(' ', $expected));
                    } else {
                        $phpcsFile
                            ->fixer
                            ->replaceToken(($index['arrow'] - 1), str_repeat(' ', $expected));
                    }
                }
            ...
            if ($tokens[$index['value']]['column'] !== $valueStart) {
                ...
                if ($fix === true) {
                    if ($found === 'newline') {
                        ...
                        $phpcsFile
                            ->fixer
                            ->replaceToken(($index['value'] - 1), str_repeat(' ', $expected));
                        $phpcsFile
                            ->fixer
                            ->endChangeset();
                    } elseif ($found === 0) {
                        $phpcsFile
                            ->fixer
                            ->addContent(($index['value'] - 1), str_repeat(' ', $expected));
                    } else {
                        $phpcsFile
                            ->fixer
                            ->replaceToken(($index['value'] - 1), str_repeat(' ', $expected));
